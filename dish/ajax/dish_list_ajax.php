...
<?php
    include $_SERVER['DOCUMENT_ROOT'] . "/ip/config.php";
		...
		$mainSQL= "SELECT dish.id,dishtype.dishtype,dish.dish,dish.price,dish.image_name FROM dish,dishtype WHERE dish.dishtype_id = dishtype.id ";


if ($_POST["searchSqlString"] != '') {
		$mainSQL = $mainSQL." ".$_POST["searchSqlString"]." order by dish.id DESC";
	} else {
		$mainSQL = $mainSQL." order by dish.id DESC";
	}
...
echo " <div class='btn-group'>
				  <button type='button' class='btn btn-primary dish' id='add' data-id='".$row['id']."' data-action='add'>ADD</button>
				  </div>
				   ";


echo " <div class='btn-group'>
				  <button type='button' class='btn btn-primary dish' id='search' data-id='".$row['id']."' data-action='search'>SEARCH</button>
				  </div>
				   ";
	?>
...
$query = $mysqli->query($mainSQL." LIMIT $start_from, $record_per_page");
        while($row = mysqli_fetch_assoc($query)) {
			...
			echo "<td><div class='btn-group'>
				  <button type='button' class='btn btn-primary dish' id='edit_dish' data-id='".$row['id']."' data-action='edit'>Edit</button>
				  </div>
				  </td>";


				  echo "<td><div class='btn-group'>
				  <button type='button' class='btn btn-danger dish_delete' id='delete' data-id='".$row['id']."' data-action='delete'>Delete</button>
				  </div>
				  </td>";
			echo "<td>" . $row['id'] . "</td>";
			...
			echo "<td>
			 ...
			 if (empty($row['image_name'])) {
				 ...
				 echo "<td>
						<img src='/ip/dish/images/".$row['image_name']."' class='rounded img-thumbnail' alt='".$row['id']."' id='recordImage_".$row['id']."' width='150' height='100'>
					  </td>";
			 }
			echo "</tr>";
        }
