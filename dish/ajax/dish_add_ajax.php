...
<?php
    ...
    $dish1         = trim(htmlspecialchars(stripslashes(cl($_POST["dish1"]))));
    $price = trim(htmlspecialchars(stripslashes(cl($_POST["price"]))));
		$detail = trim(htmlspecialchars(stripslashes(cl($_POST["detail"]))));

	$query = mysqli_query($mysqli, "INSERT dish (price,dishtype_id,dish,detail) VALUES ('$price','$dishtype_id','$dish1','$detail')");
